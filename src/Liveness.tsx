...
let detectionsList: DetectionActions[] = [
  "BLINK",
  "TURN_HEAD_LEFT",
  "TURN_HEAD_RIGHT",
  "NOD",
...
type OnTestCompleteProps = (image: CameraCapturedPicture) => void

type Props = {
  detectionsList?: DetectionActions[],
  onTestComplete?: OnTestCompleteProps
}

export default function Liveness(props: Props) {
  ...
  const onFacesDetected = async (result: FaceDetectionResult) => {
  ...
  React.useEffect(() => {
    (async () => {
      if (state.processComplete) {
        let picture = await cameraRef?.current?.takePictureAsync({ skipProcessing: true })
        props?.onTestComplete && props?.onTestComplete(picture)
      }
    })()
